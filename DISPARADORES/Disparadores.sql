...
-- CREAMOS UN DISPARADOR QUE SE ENCARGA DE ASOCIAR A UN ALUMNO A CADA
-- UNO DE LOS CRITERIOS DE EVALUACIÓN DE UNA PRUEBA
CREATE OR REPLACE TRIGGER PON_AL_PRU
AFTER INSERT ON PRU_UD_CE
FOR EACH ROW
DECLARE
  CURSOR CUR_ALUMNOS IS SELECT A.DNI, M.MODU FROM ALUMNO A, MATRICULA M
  WHERE A.DNI=M.ALU;
BEGIN
  FOR V_ALU IN CUR_ALUMNOS LOOP
    IF V_ALU.MODU = :NEW.MODU THEN
      INSERT INTO AL_PRU (ALU,PRU,MODU,CE,RA) VALUES
      (V_ALU.DNI, :NEW.PRU, :NEW.MODU, :NEW.CE, :NEW.RA);
    END IF;
  END LOOP;
END;
/